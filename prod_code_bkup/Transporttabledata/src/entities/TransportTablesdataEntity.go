...
type InsertresponceEntity struct {
	Status  bool   `json:"success"`
	Message string `json:"message"`
}
type ResultofalltableEntity struct {
	Status   bool           `json:"success"`
	Message  string         `json:"message"`
	Response []ResultEntity `json:"tablesdata"`
}
type Tablesandtype struct {
...
type Transporttable struct {
	...
	Table             []Tablesandtype `json:"table"`
}
...
func (w *Transporttable) FromJSON(r io.Reader) error {
	e := json.NewDecoder(r)
	return e.Decode(w)
}
func (w *Uploadentity) FromJSON(r io.Reader) error {
	e := json.NewDecoder(r)
	return e.Decode(w)
}
