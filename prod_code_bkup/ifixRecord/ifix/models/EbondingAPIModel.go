...
func TDLUpdateTicket(tz *entities.EbondingRecordEntity, db *sql.DB) error {
	...
	if tz.EbondingModuleSeq == 5 {
		currstatusSeq, _ := dataAccess.GetUpdatedTicketStatusFromLog(tz)

		if currstatusSeq == tz.StatusSeq {

			return errors.New("Status Already updated on other end!!!")
		}
		if tz.StatusSeq == 3 { //resolved
			stateID, stateIDErr := dataAccess.GetStateID(tz)
			if stateIDErr != nil {
				logger.Log.Println("stateIDErr====>", stateIDErr)
				return stateIDErr
			}
			stateWiseComment, stateWiseCommentErr := dataAccess.GetStateWiseComment(tz, 8)
			if stateWiseCommentErr != nil {
				logger.Log.Println("stateWiseCommentErr====>", stateWiseCommentErr)
				return stateIDErr
			}
			workNote = stateWiseComment
			tdlState = stateID
		} else if tz.StatusSeq == 10 { //reopen
			stateID, stateIDErr := dataAccess.GetStateID(tz)
			if stateIDErr != nil {
				logger.Log.Println("stateIDErr====>", stateIDErr)
				return stateIDErr
			}
			stateWiseComment, stateWiseCommentErr := dataAccess.GetStateWiseComment(tz, 15)
			if stateWiseCommentErr != nil {
				logger.Log.Println("stateWiseCommentErr====>", stateWiseCommentErr)
				return stateIDErr
			}
		...
		} else if tz.StatusSeq == 11 { //cancel
			stateID, stateIDErr := dataAccess.GetStateID(tz)
			if stateIDErr != nil {
				logger.Log.Println("stateIDErr====>", stateIDErr)
				return stateIDErr
			}
			stateWiseComment, stateWiseCommentErr := dataAccess.GetStateWiseComment(tz, 16)
			if stateWiseCommentErr != nil {
				logger.Log.Println("stateWiseCommentErr====>", stateWiseCommentErr)
				return stateIDErr
			}
	...
	if tz.EbondingModuleSeq == 4 {
		if len(tz.UploadedFileName) > 0 || len(tz.OriginalFileName) > 0 {
			...
			content, err := ioutil.ReadFile(filePath)
			if err != nil {
				logger.Log.Println("ReadFile Err==========>", err)
				return err
			}
			contentEncode := base64.StdEncoding.EncodeToString(content)
			attachEncode := entities.EbondingTDLFileAttachmentDetails{}
	...
	ebondingTDLUpdateRecord.StateID = tdlState
