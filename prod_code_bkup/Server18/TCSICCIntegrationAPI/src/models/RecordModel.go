...
func CreateRecordModel(data *entities.RecordEntity, db *sql.DB) (string, int64, error) {
					...
					*/
					...
					if len(statusdet) > 0 {
						...
						*/
						...
						logger.Log.Print("\nfinal ", recordEntity)
						log.Print("\nfinal ", recordEntity)
						taskticketID, taskinsertedID, modelResponseError := AddRecordModelAction(&recordEntity, db, parentdiffid)
						if modelResponseError != nil {
							//return "", 0, errors.New("Something went wrong")
...
func AddRecordModelAction(data *entities.RecordEntity, dbcon *sql.DB, RecordtypeID int64) (string, int64, error) {
	...
	var ticketID string
	var recordset = data.RecordSets
	logger.Log.Println("workingdiffid for Zero Only --->", workingdiffid)
		...
		if len(recordset[i].Type) > 0 {
			for j := 0; j < len(recordset[i].Type); j++ {
				if recordset[i].Type[j].ID == data.Workingcatlabelid {
					...
					wf := entities.Workflowentity{}
					wf.Clientid = data.ClientID
					wf.Mstorgnhirarchyid = data.Mstorgnhirarchyid
					wf.Recorddifftypeid = recordset[i].Type[j].ID
					wf.Recorddiffid = recordset[i].Type[j].Val
					var err1 error
					_, workflowflag, err1, _ = Checkworkflow(&wf, dbcon)
					if err1 != nil {
						logger.Log.Println(err1)
						return "", 0, errors.New("Error in checkworkflow....")
					}
	...
	if workflowflag == true {
		...
		if insertedID > 0 {
			...
			if err != nil {
				...
				go CallMoveworkflowWithAPI(data, isApproveworkflow, isApproveflag, workingdiffid, childseq, dbcon, insertedID)
...
func CreateRecordNumber(data *entities.RecordEntity, insertedID int64, Transactionnumber string, recordtype int64, dbcon *sql.DB) (string, error) {
	...
	for {
		...
		if count == 0 {
			err = dataAccess.UpdateRecordIDForDuplicateCheck(insertedID, ticketID)
			if err != nil {
				logger.Log.Println(err)
...
func CreateRecordNumberNew(data *entities.RecordEntity, insertedID int64, Transactionnumber string, recordtype int64, dbcon *sql.DB) (string, error) {
	...
	if Isclient == 0 {
		...
		// if len(ticketID) > 0 {
		// 	err = dataAccess.UpdateRecordID(insertedID, ticketID, tx)
		// 	if err != nil {
		// 		logger.Log.Println(err)
		// 		tx.Rollback()
		// 		//return "", errors.New("Ticket random number updation failures....")
...
func CreateRecordNumberNew1(data *entities.RecordEntity, insertedID int64, Transactionnumber string, recordtype int64, dbcon *sql.DB) (string, error) {
	var ticketID string
	var typediffID int64
	var ID int64
	dataAccess := dao.DbConn{DB: dbcon}
	for {
		...
		if Isclient == 0 {
			...
			if err != nil {
				...
				logger.Log.Println(err)
				tx.Rollback()
				return "", errors.New("Commit Fails............")
			}
			//logger.Log.Println(ID)
		} else {
...
func CallMoveworkflowWithAPI(data *entities.RecordEntity, isApproveworkflow int64, isApproveflag int64, workingdiffid int64, childseq int64, dbcon *sql.DB, insertedID int64) error {
	dataAccess := dao.DbConn{DB: dbcon}
	if workingdiffid > 0 && childseq != 3 {
		if isApproveflag == 1 && childseq == 5 {
			...
			postBody, _ := json.Marshal(map[string]int64{
				...
				"mstgroupid":           data.Originalusergroupid,
				"mstuserid":            data.Originaluserid,
				"userid":               data.Originaluserid,
				"createdgroupid":       data.Originalusergroupid,
		...
		if isApproveworkflow == 1 {
			if isApproveflag == 1 {
				...
				postBody, _ := json.Marshal(map[string]int64{
					...
					"mstuserid":              data.Originaluserid,
					"userid":                 data.Originaluserid,
					"creatticketIDedgroupid": data.Originalusergroupid,
				})
				b, err := json.MarshalIndent(postBody, "", "  ")
				if err != nil {
					fmt.Println(err)
...
func Checkworkflow(tz *entities.Workflowentity, dbcon *sql.DB) (int64, bool, error, string) {
	...
	*/
	processValue, err := dataAccess.GetProcessByCategoryNew(tz)
	if err != nil {
