...
func (mdao DbConn) Getfrequentissues(page *entities.RecordcommonEntity) ([]entities.FrequentRecordEntity, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&value.LastlevelID, &value.Count, &value.ParentcatID, &value.Parentcatname, &value.Recorddifftypeid, &value.Recorddiffid, &value.Seq)
		values = append(values, value)
	}
	return values, nil
}
...
func (mdao DbConn) Getfrequentissuesresolver(page *entities.RecordcommonEntity) ([]entities.FrequentRecordEntity, error) {
	logger.Log.Println("In side Getfrequentissues")
	values := []entities.FrequentRecordEntity{}
	rows, err := mdao.DB.Query(frequentresolver, page.ClientID, page.Mstorgnhirarchyid, page.Recorddifftypeid, page.Recorddiffid, page.ClientID, page.Mstorgnhirarchyid, page.Recorddifftypeid, page.Recorddiffid, page.Usergroupid)
	defer rows.Close()
	if err != nil {
		logger.Log.Println("Getfrequentissues Get Statement Prepare Error", err)
		return values, err
