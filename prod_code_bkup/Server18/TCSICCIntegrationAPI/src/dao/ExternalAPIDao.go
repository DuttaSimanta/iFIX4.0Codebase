...
func (mdao DbConn) GetTermnamesbystatusID(ClientID int64, OrgnIDTypeID int64, StatusID int64) ([]string, error) {
	logger.Log.Println("In side GetDifferentiationdstatustls")
	var values []string
	...
	rows, err := mdao.DB.Query(sql, ClientID, OrgnIDTypeID, StatusID)
	defer rows.Close()
	if err != nil {
		logger.Log.Println("GetDifferentiationdstatustls Get Statement Prepare Error", err)
		return values, err
