...
func GetEmailIDs(db *sql.DB, clientID int64, orgID int64, recordID int64,
	...
	if sendToCreator == 1 && sendToOriginalCreator == 1 && creatorID == originalCreatorID && sendToAssignee == 1 && sendToAssigneeGroup == 1 && sendToAssigneeGroupMember == 1 {
		...
		getEmailAddOriginalCreatorquery := "select useremail from mstclientuser where  id in (select userid from mstgroupmember where groupid  in(select id from mstclientsupportgroup where clientid=? and mstorgnhirarchyid=? and supportgrouplevelid=1 and deleteflg=0) and userid = ? and deleteflg =0) and deleteflag = 0"
	...
	//CCGrp Emails
	var getCCGroupEmailAddquery = "select grpid,email from mstclientsupportgroup where grpid in(select groupid from mstnotificationrecipients where clientid=? and mstorgnhirarchyid=? and activeflg=1 and deleteflg=0 and notificationtemplateid=? and recipienttype='CC')and deleteflg=0 "
	...
	for getCCGroupEmailResultset.Next() {
		...
		if len(grpEmail) == 0 {
			getUserEmailOfGrpQuery := "select useremail from mstclientuser where id in(select userid from mstgroupmember where clientid=? and mstorgnhirarchyid=? and groupid=? and activeflg=1 and deleteflg=0) and deleteflag=0"
...
func StatusChangeEvent(db *sql.DB, workingCategoryID int64, requestData map[string]interface{}) error {
	...
	getConvertedQuery := "select recordtrackvalue from trnreordtracking where recordtermid in (SELECT id FROM mstrecordterms where clientid=? and mstorgnhirarchyid=? and seq=79 and deleteflg =0) and recordid=? and deleteflg=0"
	...
	//var groupID int64
	getCreatorIDQuery := "select userid,originaluserid from trnrecord where clientid=? and mstorgnhirarchyid=? and id=? and deleteflg =0"
...
func PriorityChangeEvent(db *sql.DB, workingCategoryID int64, requestData map[string]interface{}) error {
	...
	getCurrentPrioritySeq := "select b.seqno from maprecordtorecorddifferentiation a,mstrecorddifferentiation b where a.clientid=? and a.mstorgnhirarchyid=? and a.recordid=? and a.deleteflg =0 and b.deleteflg =0 and a.recorddifftypeid=5 and a.recorddiffid=b.id order by a.id desc limit 1"
	...
	getPreviousPrioritySeq := "select b.seqno from maprecordtorecorddifferentiation a,mstrecorddifferentiation b where a.clientid=? and a.mstorgnhirarchyid=? and a.recordid=? and a.deleteflg = 0 and b.deleteflg =0 and a.recorddifftypeid=5 and a.recorddiffid=b.id order by a.id desc limit 1 offset 1"
...
func MailBodyFormationFromTemplate(requestData map[string]interface{}) error {
	...
	getWorkingCategoryIDQuery := "select recorddiffid from maprecordtorecorddifferentiation where clientid=? and mstorgnhirarchyid=? and recordid=? and isworking=1 and islatest=1 and deleteflg=0"
