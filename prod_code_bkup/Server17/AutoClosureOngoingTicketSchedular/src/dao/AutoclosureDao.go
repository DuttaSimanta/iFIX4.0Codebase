...
func UpdateDefectiveFlag(db *sql.DB, ClientID int64, Mstorgnhirarchyid int64, RecordID int64) error {
	var sql = "UPDATE mstrecordautoclosure SET defective='Y' WHERE clientid=? AND mstorgnhirarchyid=? AND recordid=?"
...
// func GetAutoClosureConfigList(db *sql.DB) ([]entities.AutoClosureConfig, error) {
// 	Logger.Log.Println("In side GetResolvedRecordsInfo model function")
// 	t := []entities.AutoClosureConfig{}
// 	var Sql = "SELECT clientid,id as mstorgnid,autoclose,coalesce(autoclosefromdt,'') autoclosefromdt,coalesce(autoclosetodt,'') autoclosetodt FROM mstorgnhierarchy where autoclose=1 and  deleteflg=0"
...
// 	}
// 	for rows.Next() {
// 		value := entities.AutoClosureConfig{}
// 		rows.Scan(&value.ClientID, &value.MstorgnhirarchyID, &value.IsAutoclose, &value.AutocloseFromdate, &value.AutocloseTodate)
// 		t = append(t, value)
// 	}
// 	return t, nil
// }
func GetResolvedRecordsInfo(db *sql.DB, ongoingdate string) ([]entities.RecordInfo, error) {
	Logger.Log.Println("In side GetResolvedRecordsInfo model function")
	...
	//	var Sql = " select a.clientid,a.mstorgnhirarchyid,a.recordid,a.recordstageid,b.closuredate,d.recorddifftypeid,d.recorddiffid,c.usergroupid,c.userid,e.mststateid from maprecordtorecorddifferentiation a , mstrecordautoclosure b, trnrecord c,maprecordtorecorddifferentiation d,maprecordstatetodifferentiation e where  a.recorddiffid in (select id from mstrecorddifferentiation where recorddifftypeid=3 and seqno=3 and deleteflg=0 and activeflg=1 ) and b.islatest=1 and a.islatest=1 and d.isworking=1 and d.islatest=1 and d.recordid=a.recordid and a.recorddifftypeid=3 and a.recordid=b.recordid and b.closureflag='N' AND  a.recordid= c.id and a.recorddiffid=e.recorddiffid and a.clientid=? and a.mstorgnhirarchyid=? and  FROM_UNIXTIME(b.closuredate) between ? and ? limit 10"
	// var Sql = " select a.clientid,a.mstorgnhirarchyid,a.recordid,a.recordstageid,b.closuredate,d.recorddifftypeid,d.recorddiffid,c.usergroupid,c.userid,e.mststateid from maprecordtorecorddifferentiation a , mstrecordautoclosure b, trnrecord c,maprecordtorecorddifferentiation d,maprecordstatetodifferentiation e where  a.recorddiffid in (select id from mstrecorddifferentiation where recorddifftypeid=3 and seqno=3 and deleteflg=0 and activeflg=1 ) and b.islatest=1 and a.islatest=1 and d.isworking=1 and d.islatest=1 and d.recordid=a.recordid and a.recorddifftypeid=3 and a.recordid=b.recordid and b.closureflag='N' AND defective='N' AND  a.recordid= c.id and a.recorddiffid=e.recorddiffid and a.clientid=? and a.mstorgnhirarchyid=? and  b.closuredt between ? and ? limit 200"
	var Sql = `select a.clientid,a.mstorgnhirarchyid,a.recordid,a.recordstageid,b.closuredate,d.recorddifftypeid,d.recorddiffid,
	c.usergroupid,c.userid,e.mststateid,f.recorddifftypeid as tickettypedifftypeid,f.recorddiffid as tickettypediffid from maprecordtorecorddifferentiation a , mstrecordautoclosure b, trnrecord c,
   ...
   b.closureflag='N' AND defective='N' AND  a.recordid= c.id and a.recorddiffid=e.recorddiffid and a.clientid=2 and 
   b.closuredt>='2022-11-01 00:00:00'  and b.closuredt<=ADDTIME(now(), '5:30:0.0') and a.recordid=f.recordid and
	f.recorddifftypeid=2 and f.islatest=1 and f.recorddiffid in (select id from mstrecorddifferentiation where recorddifftypeid=2 and 
   seqno in (1,2) and deleteflg=0 and activeflg=1 ) limit 10;`
	Logger.Log.Println()
	...
	Logger.Log.Println("ROWS:", rows)

	for rows.Next() {
		value := entities.RecordInfo{}
		rows.Scan(&value.ClientID, &value.MstorgnhirarchyID, &value.RecordID, &value.RecordStageID, &value.Closuredate, &value.WorkingDifftypeID, &value.WorkingDiffID, &value.CreatedgrpID, &value.MstuserID, &value.PreviousStateID, &value.TickettypeDiffTypeId, &value.TickettypeDiffId)
		t = append(t, value)
	}
	return t, nil
}
