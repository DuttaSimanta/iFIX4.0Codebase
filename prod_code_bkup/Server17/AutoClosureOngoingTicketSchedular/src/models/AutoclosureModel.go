...
func Autoclosure() {
	...
	values, err := dao.GetResolvedRecordsInfo(db, props["OngingDate"])
	if err != nil {
	...
	for i := 0; i < len(values); i++ {
		...
		logger.Log.Println("Total Tickets  --------->", len(values))
		logger.Log.Println("===================================================Ticket ID is ===========================>", values[i])
		...
		if sequance == 3 {
			...
			logger.Log.Println("Record status request body -->", string(postBody))
