...
func (mdao DbConn) GetSRBugRecords() ([]entities.RecordDetailsEntity, error) {
	logger.Log.Println("In side GetTermvalueagainsttermid")
	values := []entities.RecordDetailsEntity{}
//	latestrecordcomment := "SELECT a.clientid,a.mstorgnhirarchyid,a.recordid,b.id as requestid FROM iFIX.maprequestorecord a,mstrequest b,maprecordstatetodifferentiation c,maprecordtorecorddifferentiation d,trnrecord e,maprecordtorecorddifferentiation h,mstrecorddifferentiation i where a.mstrequestid=b.id and b.currentstateid=c.mststateid and b.clientid=c.clientid and b.mstorgnhirarchyid=c.mstorgnhirarchyid and c.recorddifftypeid=3 and a.recordid=d.recordid and d.clientid=a.clientid and d.mstorgnhirarchyid=a.mstorgnhirarchyid and  d.recorddifftypeid=3 and d.islatest=1 and c.recorddiffid<>d.recorddiffid and d.recordstageid=a.recordstageid and a.recordid=e.id and e.deleteflg=0 and a.deleteflg=0 and b.deleteflg=0 and c.deleteflg=0 and d.deleteflg=0  and a.recordid=h.recordid and a.clientid=h.clientid and a.mstorgnhirarchyid=h.mstorgnhirarchyid and h.recorddifftypeid=2 and h.islatest=1 and h.recorddiffid=i.id  and i.name='Request' and a.recordid not in (select recordid from defectticketlog where a.clientid=defectticketlog.clientid and a.mstorgnhirarchyid=defectticketlog.mstorgnhirarchyid and isdefect='Y') limit 15;"
//	latestrecordcomment := "SELECT a.clientid,a.mstorgnhirarchyid,a.recordid,b.id as requestid FROM maprequestorecord a,mstrequest b,maprecordstatetodifferentiation c,maprecordtorecorddifferentiation d,trnrecord e,maprecordtorecorddifferentiation h,mstrecorddifferentiation i,mstrecorddifferentiation f,mstrecorddifferentiation g,mstorgnhierarchy n where a.mstrequestid=b.id and b.currentstateid=c.mststateid and b.clientid=c.clientid and b.mstorgnhirarchyid=c.mstorgnhirarchyid and c.recorddifftypeid=3 and a.recordid=d.recordid and d.clientid=a.clientid and d.mstorgnhirarchyid=a.mstorgnhirarchyid and  d.recorddifftypeid=3 and d.islatest=1 and c.recorddiffid<>d.recorddiffid and d.recordstageid=a.recordstageid and a.recordid=e.id and e.deleteflg=0 and a.deleteflg=0 and b.deleteflg=0 and c.deleteflg=0 and d.deleteflg=0  and a.recordid=h.recordid and a.clientid=h.clientid and a.mstorgnhirarchyid=h.mstorgnhirarchyid and h.recorddifftypeid=2 and h.islatest=1 and h.recorddiffid=i.id  and i.name='Request' and a.recordid not in (select recordid from defectticketlog where a.clientid=defectticketlog.clientid and a.mstorgnhirarchyid=defectticketlog.mstorgnhirarchyid and isdefect='Y') and c.recorddiffid=f.id and d.recorddiffid=g.id and n.id=e.mstorgnhirarchyid and g.seqno=12 and f.seqno=15 order by e.id desc limit 15;"
	latestrecordcomment := "SELECT a.clientid,a.mstorgnhirarchyid,a.recordid,b.id as requestid FROM maprequestorecord a,mstrequest b,maprecordstatetodifferentiation c,maprecordtorecorddifferentiation d,trnrecord e,maprecordtorecorddifferentiation h,mstrecorddifferentiation i,mstrecorddifferentiation f,mstrecorddifferentiation g,mstorgnhierarchy n where a.mstrequestid=b.id and b.currentstateid=c.mststateid and b.clientid=c.clientid and b.mstorgnhirarchyid=c.mstorgnhirarchyid and c.recorddifftypeid=3 and a.recordid=d.recordid and d.clientid=a.clientid and d.mstorgnhirarchyid=a.mstorgnhirarchyid and  d.recorddifftypeid=3 and d.islatest=1 and c.recorddiffid<>d.recorddiffid and d.recordstageid=a.recordstageid and a.recordid=e.id and e.deleteflg=0 and a.deleteflg=0 and b.deleteflg=0 and c.deleteflg=0 and d.deleteflg=0  and a.recordid=h.recordid and a.clientid=h.clientid and a.mstorgnhirarchyid=h.mstorgnhirarchyid and h.recorddifftypeid=2 and h.islatest=1 and h.recorddiffid=i.id  and i.name='Request' and a.recordid not in (select recordid from defectticketlog where a.clientid=defectticketlog.clientid and a.mstorgnhirarchyid=defectticketlog.mstorgnhirarchyid and isdefect='Y') and c.recorddiffid=f.id and d.recorddiffid=g.id and n.id=e.mstorgnhirarchyid and g.seqno in (12) and f.seqno in(15) order by e.id desc limit 2;"
...
func (mdao DbConn) GetBugRecords() ([]entities.RecordDetailsEntity, error) {
	logger.Log.Println("In side GetTermvalueagainsttermid")
	values := []entities.RecordDetailsEntity{}
	latestrecordcomment := "SELECT a.clientid,a.mstorgnhirarchyid,a.recordid,b.id as requestid FROM iFIX.maprequestorecord a,mstrequest b,maprecordstatetodifferentiation c,maprecordtorecorddifferentiation d,trnrecord e,maprecordtorecorddifferentiation h,mstrecorddifferentiation i where a.mstrequestid=b.id and b.currentstateid=c.mststateid and b.clientid=c.clientid and b.mstorgnhirarchyid=c.mstorgnhirarchyid and c.recorddifftypeid=3 and a.recordid=d.recordid and d.clientid=a.clientid and d.mstorgnhirarchyid=a.mstorgnhirarchyid and  d.recorddifftypeid=3 and d.islatest=1 and c.recorddiffid<>d.recorddiffid and d.recordstageid=a.recordstageid and a.recordid=e.id and e.deleteflg=0 and a.deleteflg=0 and b.deleteflg=0 and c.deleteflg=0 and d.deleteflg=0  and a.recordid=h.recordid and a.clientid=h.clientid and a.mstorgnhirarchyid=h.mstorgnhirarchyid and h.recorddifftypeid=2 and h.islatest=1 and h.recorddiffid=i.id  and i.name='Incident' and a.recordid not in (select recordid from defectticketlog where a.clientid=defectticketlog.clientid and a.mstorgnhirarchyid=defectticketlog.mstorgnhirarchyid and isdefect='Y') order by e.id asc limit 15;"
	rows, err := mdao.DB.Query(latestrecordcomment)
	defer rows.Close()
	if err != nil {
		logger.Log.Println("GetTermvalueagainsttermid Get Statement Prepare Error", err)
...
func (mdao DbConn) GetWorkflowState(page entities.RecordDetailsEntity) ([]entities.StatedetailEntity, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&value.Id, &value.Stateid, &value.Userid, &value.Createdgroupid, &value.Statusid, &value.Statusname, &value.StatusSeq)
		values = append(values, value)
	}
	logger.Log.Println(values)
	return values, nil
...
func (dbc DbConn) InsertCommentOrErr(tz *entities.ErrorEntity) (int64, error) {
	...
	sql := "INSERT INTO defectticketlog (clientid, mstorgnhirarchyid, recordid, stateid, requestjson, responsejson, comment) VALUES (?,?,?,?,?,?,?)"
	...
	res, err := stmt.Exec(tz.ClientID, tz.Mstorgnhirarchyid, tz.RecordID, tz.Stateid, tz.Requestjson, tz.Responsejson, tz.Comment)
	if err != nil {
...
func (dbc DbConn) DeleteDefectTkt() (int64, error) {
	sql := "delete from defectticketlog where isdefect='Y'"
	stmt, err := dbc.DB.Prepare(sql)
	if err != nil {
		logger.Log.Println("Prepare Statement>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>history table id----------22222222222222222222222222222222------------>", err)
		return 0, err
	...
	rowsaffacted, err := res.RowsAffected()
	return rowsaffacted, nil
}
