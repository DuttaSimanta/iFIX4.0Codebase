...
func InsertMultipleTermvalues(tz *entities.RecordmultiplecommonEntity) (int64, bool, error, string) {
	...
	for i := 0; i < len(tz.Details); i++ {
		...
		if len(v.Insertedvalue) > 0 {
			...
			if termseq == 33 {
				err = dao.UpdateClosureCode(tx, tz.ClientID, tz.Mstorgnhirarchyid, tz.RecordID, tz.Details[i].Insertedvalue)
				if err != nil {
					log.Println("error is ----->", err)
					tx.Rollback()
					//	dbcon.Close()
			...
			if termseq == 35 {
				err = dao.UpdateClosureComment(tx, tz.ClientID, tz.Mstorgnhirarchyid, tz.RecordID, tz.Details[i].Insertedvalue)
				if err != nil {
					log.Println("error is ----->", err)
					tx.Rollback()
					//	dbcon.Close()
				...
				for i := 0; i < len(tz.Details); i++ {
					...
					if len(v.Insertedvalue) > 0 {
						...
						if releationID > 0 {
							...
							termseq := hashmap[tz.Details[i].ID]
							if termseq == 33 {
								err = dao.UpdateClosureCode(tx, tz.ClientID, tz.Mstorgnhirarchyid, childids[k], tz.Details[i].Insertedvalue)
								if err != nil {
									log.Println("error is ----->", err)
									tx.Rollback()
									//	dbcon.Close()
							...
							if termseq == 35 {
								err = dao.UpdateClosureComment(tx, tz.ClientID, tz.Mstorgnhirarchyid, childids[k], tz.Details[i].Insertedvalue)
								if err != nil {
									log.Println("error is ----->", err)
									tx.Rollback()
									//	dbcon.Close()
					...
					for i := 0; i < len(tz.Details); i++ {
						...
						if len(v.Insertedvalue) > 0 {
							...
							if releationID > 0 {
								...
								termseq := hashmap[tz.Details[i].ID]
								if termseq == 33 {
									err = dao.UpdateClosureCode(tx, tz.ClientID, tz.Mstorgnhirarchyid, parentid[k], tz.Details[i].Insertedvalue)
									if err != nil {
										log.Println("error is ----->", err)
										tx.Rollback()
										//	dbcon.Close()
								...
								if termseq == 35 {
									err = dao.UpdateClosureComment(tx, tz.ClientID, tz.Mstorgnhirarchyid, parentid[k], tz.Details[i].Insertedvalue)
									if err != nil {
										log.Println("error is ----->", err)
										tx.Rollback()
										//	dbcon.Close()
...
func Getrecordcount(tz *entities.RecordcommonEntity) (entities.RecordcountEntity, bool, error, string) {
	...
	if aging > 0 {
		err = dataAccess.UpdateAging(tz.ClientID, tz.Mstorgnhirarchyid, tz.RecordID, aging)
		if err != nil {
			log.Println("error is ----->", err)
		}
	}
...
func GetSLADuetimeCalculation(page *entities.RecordcommonEntity) (bool, error, string) {
	...
	SLADueTimeCalculation(page.RecordID, 0, 1, 3, datetime.Format("2006-01-02 15:04:05"), page.ClientID, page.Mstorgnhirarchyid, res.RecordtypeID, res.WorkingcatID, res.PriorityID, "", grpID)
