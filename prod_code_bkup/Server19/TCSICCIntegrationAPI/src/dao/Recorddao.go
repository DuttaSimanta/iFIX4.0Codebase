...
func InsertTrnRecord(tx *sql.Tx, rec *entities.RecordEntity, lognumber string, Recordcode string) (int64, error) {
	...
	defer stmt.Close()
	res, err := stmt.Exec(rec.ClientID, rec.Mstorgnhirarchyid, rec.Recordname, rec.Recordesc, rec.Requesterinfo, rec.CreateduserID, rec.CreatedusergroupID, rec.Originaluserid, rec.Originalusergroupid, rec.Requestername, rec.Requesteremail, rec.Requestermobile, rec.Requesterlocation, rec.Source, Recordcode)
	if err != nil {
		logger.Log.Println(err)
...
func UpdateFollowupcount(tx *sql.Tx, ClientID int64, OrgnID int64, RecordID int64) error {
	var sql = "UPDATE recordfulldetails SET followupdatetime=now()  WHERE clientid=? AND mstorgnhirarchyid=? AND recordid=?"
	stmt, err := tx.Prepare(sql)
	if err != nil {
		logger.Log.Println(err)
		return errors.New("SQL Prepare Error")
...
func (mdao DbConn) UpdateRecordID(lastInsertedID int64, recordno string, tx *sql.Tx) error {
	logger.Log.Println("updatetrnrecord query -->", updatetrnrecord)
	logger.Log.Println("parameters -->", recordno, lastInsertedID)
...
func (mdao DbConn) UpdateRecordIDForDuplicateCheck(lastInsertedID int64, recordno string) error {
	logger.Log.Println("updatetrnrecord query -->", updatetrnrecord)
	logger.Log.Println("parameters -->", recordno, lastInsertedID)
...
// 	var Recordno string
// 	var ID int64
// 	var sql = "SELECT id,code FROM mstrecordcode WHERE recorddifftypeid=? AND recorddiffid=? AND isuse=0 AND activeflg=1 AND deleteflg=0 limit 1 " //FOR UPDATE
// 	stmt, error := mdao.DB.Prepare(sql)
// 	if error != nil {
// 		logger.Log.Println("Exception in GetParentRecordNo Prepare Statement..")
// 		return ID, Recordno, error
...
// 	logger.Log.Println("UpdateRecordnoTB ID ------------->", ID)
// 	var sql = "UPDATE mstrecordcode SET isuse=1 WHERE id=?"
// 	stmt, err := mdao.DB.Prepare(sql)
// 	if err != nil {
// 		logger.Log.Println(err)
...
func (mdao DbConn) GetProcessByCategoryNew(tz *entities.Workflowentity) (entities.Workflowentity, error) {
	log.Println("In side dao")
	value := entities.Workflowentity{}
	var query = "SELECT mstprocessid as Processid from mstprocessrecordmap where clientid=? and mstorgnhirarchyid=? and recorddifftypeid = ? and recorddiffid =? and deleteflg=0 and activeflg=1"
	err := mdao.DB.QueryRow(query, tz.Clientid, tz.Mstorgnhirarchyid, tz.Recorddifftypeid, tz.Recorddiffid).Scan(&value.Processid)
	switch err {
	case sql.ErrNoRows:
		value.Processid = 0
		return value, nil
...
func (mdao DbConn) Checkprocesscomplete(tz *entities.Workflowentity) ([]entities.Workflowentity, error) {
	...
	var query = "select iscomplete from mstprocessdetails where clientid=? and mstorgnhirarchyid=? and processid=? and activeflg=1 and deleteflg=0"
	values := []entities.Workflowentity{}
	rows, err := mdao.DB.Query(query, tz.Clientid, tz.Mstorgnhirarchyid, tz.Processid)
